...
join_tibbles <- function(years = list(...), num_var_list = 
                               list(c("AGEP", "PWGTP"), c("AGEP", "PWGTP")),
                             cat_var_list = list(c("SEX", "SEX")),
                             geo_lev_list = list("all", "all"),
                             geo_lev_value_list = list("", "")){
  for (year in years){
    if (!(year %in% c(2010:2022))) {
    stop("Year must be between 2010 and 2022!")
    }
  ...
  tibble_1 <- url_to_tibble(pums_url_builder(years[[1]] = years, num_var_list[[1]] = num_vars,
                            cat_var_list[[1]] = cat_vars, geo_lev_list[[1]] = geo_lev,
                            geo_lev_value_list[[1]] = geo_lev_value))
  tibble_2 <- url_to_tibble(pums_url_builder(years[[2]] = years, num_var_list[[2]] = num_vars,
                            cat_var_list[[2]] = cat_vars, geo_lev_list[[2]] = geo_lev,
                            geo_lev_value_list[[2]] = geo_lev_value))
 
  return(left_join(tibble_1, tibble_2, by = "year"))
}
join_two_tibbles()
```
...
# function call
built_url <- build_url(list("FER"),list("GASP"))
built_url

url_to_tibble <- function(url = NULL) {
    raw_data <- httr::GET(url)
    parsed_data <- fromJSON(rawToChar(raw_data$content))
    parsed_tibble <- as_tibble(parsed_data)
#    return (as_tibble(parsed_data))
    parsed_tibble <- mutate(year = year)
    return(as_tibble(parsed_data))
}

url_to_tibble(built_url)

concatenate_tibbles <- function(years = list(...)){
  for (year in years){
    tibble <- url_to_tibble()
  }
...
for (i in 1:length(years)){
  nc_pums[[i]] <- get_pums(
    variables = c("PUMA", "SEX", "AGEP", "JWAP", "JWDP", "SCHL"),
    state = "NC",
    survey = "acs1",
    year = years[i]
  ) |> mutate(year = years[i])
...
example <- data.table::rbindlist(nc_pums)
view(example)
```
```{r}
#get the JWAP info
...
#nmbrs <- gregexpr("[0-9]+:[0-9]+", JWAP_values[[280]])
#JWAP_values[[280]] <- regmatches(JWAP_values[[280]], nmbrs)
#JWAP_values[[280]]

for (i in 2:length(JWAP_values)){
  if (!is.na(JWAP_values[[i]])){
    numbers <- gregexpr("[0-9]+:[0-9]+", JWAP_values[[i]])
    JWAP_values[[i]] <- regmatches(JWAP_values[[i]], numbers)
    result <- list()
    for (j in 1:length(JWAP_values[[i]][[1]])){
      result[[j]] <- gsub(":","",JWAP_values[[i]][[1]][[j]])
    }
# Convert to numeric
#  JWAP_values[[i]] <- as.numeric(unlist(result))
  
# Find the midpoint value of the times
#  JWAP_values[[i]] <- mean(JWAP_values[[i]][1],JWAP_values[[i]][2])
  
# Convert to seconds
#  JWAP_values[[i]] <- as.character(JWAP_values[[i]])
#  JWAP_values[[i]] <- lapply(strsplit(JWAP_values[[i]], ""), head, -2)
  }
}
...
#Time1 <- 
#format(as.POSIXct(Time1,format='%I:%M %p'),format="%H:%M:%S")
#typeof(JWAP_values[[280]][[1]][[1]])
#3600*as.numeric(gsub(" ","",paste(JWAP_values[[280]][[1]][[1]],JWAP_values[[280]][[1]][[2]])))
```
...
# Find the midpoint value of the times
  JWAP_values[[i]] <- (JWAP_values[[i]][[1]]+JWAP_values[[i]][[2]])/2
...
# Find the midpoint value of the times
  JWDP_values[[i]] <- (JWDP_values[[i]][[1]]+JWDP_values[[i]][[2]])/2
  }
}
